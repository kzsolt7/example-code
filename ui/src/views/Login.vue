  ...
  methods: {
    login() {
      api.post(`authenticate?username=${this.username}&password=${this.password}`).then(r => {
        if (r.status === 200) {
          this.$cookies.set("access-token", r.headers.authorization, "1000s");
          this.$cookies.set("refresh-token", r.headers.refresh, "1000s");
          this.$cookies.set("username", r.headers.username, "1000s");
